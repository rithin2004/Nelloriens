...
export const schemas = {
  ...
  SubmitLeadRequest: {
    ...
    properties: {
      ...
      inquiryType: { type: 'string', example: 'IQT00001', description: 'ID from /leads/inquiry-types/list (e.g. General, Advertising, Support).' },
  ...
  BreakingPointRequest: {
    ...
    properties: {
      ...
      expiresAt: { type: 'string', format: 'date-time', example: '2026-05-01T18:00:00.000Z', description: 'If set, this ticker point auto-hides after this timestamp. Omit for permanent points.' },
    },
  },
  ...
  CreateJobRequest: {
    ...
    properties: {
      ...
      jobType:          { type: 'string', example: 'JTP00001', description: 'ID from /jobs/types/list (e.g. Full-time, Internship).' },
      workMode:         { type: 'string', enum: ['Remote', 'On-site', 'Hybrid', 'Part-time', 'Contract'], example: 'On-site' },
  ...
  CreateSportsRequest: {
    ...
    properties: {
      ...
      category:    { type: 'string', example: 'SCT00001', description: 'ID of a sport_category document (e.g. Cricket, Kabaddi).' },
      validUpto:   { type: 'string', format: 'date-time', example: '2025-04-20T09:00:00.000Z', description: 'For upcoming events only — event is hidden after this date.' },
      ...
      city:        { type: 'string', example: 'Nellore' },
      region:      { type: 'string', example: 'Nellore District' },
    },
  },

  CreateLiveScoreRequest: {
    type: 'object',
    required: ['sportName', 'liveUrl'],
    properties: {
      sportName: { type: 'string', example: 'Cricket — AP vs Hyderabad' },
      liveUrl:   { type: 'string', example: 'https://cricbuzz.com/live-cricket-scores/12345', description: 'Redirect URL to any live score board.' },
    },
  },
  ...
  CreateStayRequest: {
    ...
    properties: {
      ...
      checkInTime:   { type: 'string', example: '12:00 PM', description: 'Standard check-in time.' },
      checkOutTime:  { type: 'string', example: '11:00 AM', description: 'Standard check-out time.' },
      websiteUrl:    { type: 'string', example: 'https://hotelminar.com' },
      bookingUrl:    { type: 'string', example: 'https://booking.example.com/hotels/hotel-minar-nellore', description: 'Generic booking URL — no third-party branding.' },
      amenities:     { type: 'array', items: { type: 'string' }, example: ['WiFi', 'AC', 'Parking', 'Restaurant'] },
      city:          { type: 'string', example: 'Nellore' },
  ...
  CreateEventRequest: {
    ...
    properties: {
      ...
      endDate:              { type: 'string',  format: 'date', example: '2025-06-03' },
      eventStartTime:       { type: 'string',  example: '09:00 AM', description: 'Daily event start time (display only).' },
      eventEndTime:         { type: 'string',  example: '09:00 PM', description: 'Daily event end time (display only).' },
      venueName:            { type: 'string',  example: 'APMC Yard, NH16, Nellore' },
      thumbnail:            { type: 'string',  example: 'https://storage.googleapis.com/nelloriens.appspot.com/events/thumbnails/EVT00001.jpg' },
      ...
      bookingUrl:           { type: 'string',  example: 'https://booking.example.com/nellore/mango-festival' },
      registrationRequired: { type: 'boolean', example: true, description: 'If true, registrationUrl must be provided.' },
      registrationUrl:      { type: 'string',  example: 'https://forms.example.com/mango-fest-2025', description: 'Registration link — required when registrationRequired is true.' },
      scope:                { type: 'string',  enum: ['nellore', 'worldwide'], example: 'nellore' },
    },
  },
  ...
  CreateMovieRequest: {
    type: 'object',
    required: ['movieName', 'language', 'genre', 'synopsis', 'status', 'runningFrom', 'thumbnail'],
    properties: {
      movieName:     { type: 'string',  example: 'Pushpa 3: The Rampage' },
      language:      { type: 'string',  example: 'LNG00001', description: 'ID from /movies/languages/list.' },
      genre:         { type: 'string',  example: 'GNR00002', description: 'ID from /movies/genres/list.' },
      synopsis:      { type: 'string',  example: 'The saga of Pushpa Raj continues as he faces his biggest enemy yet.' },
      duration:      { type: 'integer', example: 175, description: 'Runtime in minutes.' },
      director:      { type: 'string',  example: 'Sukumar' },
      cast:          { type: 'array',   items: { type: 'string' }, example: ['Allu Arjun', 'Rashmika Mandanna', 'Fahadh Faasil'] },
      theatres:      { type: 'array',   items: { type: 'string' }, example: ['THT00001', 'THT00002'], description: 'IDs of linked theatres.' },
      thumbnail:     { type: 'string',  example: 'https://storage.googleapis.com/nelloriens.appspot.com/movies/thumbnails/MOV00001.jpg' },
      poster:        { type: 'string',  example: 'https://storage.googleapis.com/nelloriens.appspot.com/movies/posters/MOV00001.jpg' },
      runningFrom:   { type: 'string',  format: 'date', example: '2026-05-01' },
      runningUntil:  { type: 'string',  format: 'date', example: '2026-05-21', description: 'Leave blank for upcoming movies.' },
      trailerUrl:    { type: 'string',  example: 'https://youtube.com/watch?v=pushpa3trailer' },
      bookingUrl:    { type: 'string',  example: 'https://booking.example.com/nellore/pushpa3', description: 'Generic booking URL — no third-party branding.' },
  ...
  CreateTransportRequest: {
    ...
    properties: {
      ...
      totalDuration:    { type: 'string',  example: '4h 35m', description: 'Total travel duration (trains and buses).' },
      runningDays:      { type: 'string',  example: 'Mon, Wed, Fri, Sun' },
      classes:          { type: 'string',  example: '2A, 3A, SL, GN' },
      fareDetails:      { type: 'array',   items: { type: 'object', properties: { class: { type: 'string', example: 'SL' }, fare: { type: 'number', example: 235 } } }, example: [{ class: 'SL', fare: 235 }, { class: '3A', fare: 610 }, { class: '2A', fare: 885 }], description: 'Structured fare breakdown by class (trains and buses).' },
      baseFare:         { type: 'number',  example: 500, description: 'Starting/base fare in INR (airports and local transport).' },
      bookingUrl:       { type: 'string',  example: 'https://booking.example.com/train/22707', description: 'Generic booking URL — no third-party branding.' },
  ...
  CreateOfferRequest: {
    ...
    properties: {
      ...
      category:           { type: 'string',  example: 'OCT00001', description: 'ID from /offers/categories/list.' },
      offerType:          { type: 'string',  example: 'OTP00001', description: 'ID from /offers/types/list (e.g. Dine-in, Online, In-store).' },
      thumbnail:          { type: 'string',  example: 'https://storage.googleapis.com/nelloriens.appspot.com/offers/thumbnails/OFR00001.jpg' },
      description:        { type: 'string',  example: 'Get 50% off on all Biryani orders above ₹500. Valid only on dine-in.' },
      ...
      minimumPurchase:    { type: 'number',  example: 500, description: 'Minimum purchase amount required to avail the offer.' },
      termsAndConditions: { type: 'string',  example: 'Valid on dine-in only. Cannot be combined with other offers. Maximum discount ₹250.' },
      ...
      location:           { type: 'string',  example: 'OLC00001', description: 'ID from /offers/locations/list.' },
      scope:              { type: 'string',  enum: ['nellore', 'worldwide'], example: 'nellore' },
    },
  },
  ...
  CreateRealEstateRequest: {
    ...
    properties: {
      ...
      furnishing:      { type: 'string',  enum: ['Furnished', 'Semi-Furnished', 'Unfurnished'], example: 'Semi-Furnished' },
      facing:          { type: 'string',  enum: ['East', 'West', 'North', 'South', 'North-East', 'North-West', 'South-East', 'South-West'], example: 'East' },
      floor:           { type: 'integer', example: 3, description: 'Current floor of the unit (for flats/apartments).' },
      totalFloors:     { type: 'integer', example: 10, description: 'Total floors in the building.' },
      ageOfProperty:   { type: 'string',  example: '2 years', description: 'Age of the property (free text).' },
      possessionDate:  { type: 'string',  format: 'date', example: '2026-07-01', description: 'Expected possession / handover date.' },
      amenities:       { type: 'array',   items: { type: 'string' }, example: ['Swimming Pool', 'Gym', 'Parking'], description: 'List of amenity IDs or names selected from /realestate/amenities/list.' },
  ...
  CreateShowtimeRequest: {
    type: 'object',
    required: ['movieId', 'startDate', 'endDate', 'timings'],
    properties: {
      movieId:    { type: 'string',  example: 'MOV00001', description: 'ID of the movie being shown.' },
      startDate:  { type: 'string',  format: 'date', example: '2026-05-01', description: 'First date of the screening run.' },
      endDate:    { type: 'string',  format: 'date', example: '2026-05-14', description: 'Last date of the screening run.' },
      timings:    { type: 'array',   items: { type: 'string' }, example: ['10:00 AM', '1:30 PM', '4:30 PM', '9:00 PM'], description: 'Show start times for each day of the run.' },
      language:   { type: 'string',  example: 'Telugu', description: 'Screening language.' },
      format:     { type: 'string',  example: '2D', description: 'Screening format (2D, 3D, IMAX, 4DX etc.).' },
      bookingUrl: { type: 'string',  example: 'https://booking.example.com/show/SHW00001', description: 'Generic booking redirect URL — no third-party branding.' },
      notes:      { type: 'string',  example: 'Holiday special — advance booking recommended.' },
    },
  },
