...
export default function StaysList() {
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try { await staysApi.delete(deleteId); toast.success('Moved to Recycle Bin'); setDeleteId(null) }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center gap-2 mb-4 px-3 py-2 rounded-lg text-xs text-amber-700 w-fit"
        style={{ background: '#FEF3C7', border: '1px solid #FDE68A' }}>
        <Star className="w-3.5 h-3.5 shrink-0" />
        <span>Max 3 Top stays per category shown on user side.</span>
      </div>
            ...
            <div className="flex flex-col gap-2 mb-5">
              {replaceCandidates.map((c) => (
                ...
                >
                  ...
                  <span className="text-sm font-medium text-slate-700 line-clamp-2">{c.hotelName || c.title}</span>
                  {replacingId === c._id && (
