...
export default function TourismCategories() {
  ...
  const fetch = () => tourismApi.getCategories().then((r) => setCategories(r.data || [])).catch(() => {})
  useEffect(() => { fetch() }, [])

  const handleCreate = async () => {
    if (!newName.trim()) return
    setAdding(true)
    try { await tourismApi.createCategory({ name: newName.trim() }); setNewName(''); fetch(); toast.success('Category created') }
  ...
  const handleUpdate = async (id) => {
    ...
    setSaving(true)
    try { await tourismApi.updateCategory(id, { name: editName.trim() }); setEditId(null); fetch(); toast.success('Updated') }
    catch (e) { toast.error(e?.response?.data?.message || 'Failed') }
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try { await tourismApi.deleteCategory(deleteId); setDeleteId(null); fetch(); toast.success('Deleted') }
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Tourism Categories" backTo="/tourism" />
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div className="p-4 flex gap-2" style={{ borderBottom: '1px solid #F1F5F9' }}>
          <input value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCreate()} placeholder="New category name" className={`flex-1 ${inp}`} style={inpStyle} onFocus={(e) => { e.target.style.borderColor = '#0a3d95'; e.target.style.boxShadow = '0 0 0 3px rgba(2,132,199,0.1)' }} onBlur={(e) => { e.target.style.borderColor = '#CBD5E1'; e.target.style.boxShadow = '' }} />
          <button onClick={handleCreate} disabled={adding || !newName.trim()} className="flex items-center gap-1.5 px-4 py-1.5 text-white text-sm font-semibold rounded-lg transition-all disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#8B5CF6,#6366F1)' }}>
            <Plus className="w-4 h-4" />
            {adding ? 'Adding…' : 'Add'}
          </button>
        </div>
        <ul>
          {categories.length === 0 && <li className="px-4 py-8 text-center text-sm text-slate-400">No categories yet.</li>}
          {categories.map((c) => (
