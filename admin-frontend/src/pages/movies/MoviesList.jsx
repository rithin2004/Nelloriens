...
export default function MoviesList() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true)
    const params = { page, limit: tab === 'upcoming' ? MAX_UPCOMING : 20 }
    if (tab === 'upcoming') params.status = 'coming_soon'
    if (tab === 'running')  params.status = 'now_showing'
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try { await moviesApi.delete(deleteId); toast.success('Deleted'); setDeleteId(null); fetchData() }
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      ...
      else            { await moviesApi.create(data);             toast.success('Created!') }
  ...
  const upcomingColumns = [
    ...
    {
      accessorKey: 'runningFrom',
      header: 'Release Date',
      cell: ({ getValue }) => {
        if (!getValue()) return <span className="text-slate-400 text-xs">TBA</span>
        return <span className="text-slate-600 text-xs font-medium">{new Date(getValue()).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
      },
    },
    {
  ...
  const runningColumns = [
    ...
    },
    {
      accessorKey: 'movieName',
      header: 'Movie',
      cell: ({ getValue }) => <span className="font-semibold text-slate-800">{truncate(getValue() || '', 40)}</span>,
    },
    { accessorKey: 'language', header: 'Language', cell: ({ getValue }) => <span className="text-slate-500 text-xs">{getValue() || '—'}</span> },
    {
      accessorKey: 'theatre',
    ...
    {
      accessorKey: 'showTimings',
      header: 'Show Timings',
      cell: ({ getValue }) => {
        const timings = getValue() || []
        if (timings.length === 0) return <span className="text-slate-300 text-xs">—</span>
        return (
    ...
    {
      accessorKey: 'bookingUrl',
      header: 'Book',
      cell: ({ getValue }) => getValue()
        ? <a href={getValue()} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold text-white" style={{ background: 'linear-gradient(135deg,#8B5CF6,#6366F1)' }}>Book ↗</a>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <ConfirmModal isOpen={!!deleteId} title="Delete Movie" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} loading={deleting} />

      <FormModal isOpen={formOpen} onClose={handleCloseForm} title={formEditId ? 'Edit Movie' : 'Add Movie'} maxWidth="max-w-2xl">
        {(formFetching || formDefaults === null) ? (
          ...
          <MovieForm defaultValues={formDefaults} onSubmit={handleFormSubmit} loading={formSubmitting} onDirtyChange={setFormDirty} />
        )}
      </FormModal>
    </div>
  )
