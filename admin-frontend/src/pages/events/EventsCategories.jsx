...
export default function EventsCategories() {
  ...
  const fetch = () => eventsApi.getCategories().then((r) => setCategories(r.data || [])).catch(() => {})
  useEffect(() => { fetch() }, [])

  const handleCreate = async () => {
    if (!newName.trim()) return
    setAdding(true)
    try { await eventsApi.createCategory({ name: newName.trim() }); setNewName(''); fetch(); toast.success('Category created') }
  ...
  const handleUpdate = async (id) => {
    ...
    setSaving(true)
    try { await eventsApi.updateCategory(id, { name: editName.trim() }); setEditId(null); fetch(); toast.success('Updated') }
    catch (e) { toast.error(e?.response?.data?.message || 'Failed') }
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try { await eventsApi.deleteCategory(deleteId); setDeleteId(null); fetch(); toast.success('Deleted') }
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Events Categories" backTo="/events" />
